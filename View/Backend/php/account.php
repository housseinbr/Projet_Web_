...
if (!isset($_SESSION['id_u'])) { // Check if user is logged in by looking for id_u in the session
    $stmt = config::getConnexion()->query("SELECT id_u FROM c_u LIMIT 1");

    if ($stmt->rowCount() > 0) {
        $user_data = $stmt->fetch(PDO::FETCH_ASSOC);
        $_SESSION['id_u'] = $user_data['id_u']; // Store id_u in session
    } else {
        // Redirect to login page if id_u is not found
...
$id_u = $_SESSION['id_u']; // Retrieve id_u from session

$stmt_user = config::getConnexion()->prepare("SELECT nom, pre, email, tel, age, genre, pwd FROM utilisateur WHERE id_u = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt_update = config::getConnexion()->prepare("UPDATE utilisateur SET nom = ?, pre = ?, email = ?, tel = ?, age = ?, genre = ?, pwd = ? WHERE id_u = ?");
    $stmt_update->execute([$nom, $pre, $email, $tel, $age, $genre, $pwd, $id_u]);

    echo "<script>alert('Mise à jour du compte avec succès');</script>";
}
?>
...
<html lang="en">
        ...
        <div class="container-fluid position-relative p-0">
            ...
            <div class="container-fluid bg-breadcrumb">
                <div class="container text-center py-5" style="max-width: 900px;">
                    <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">accder ton profile</h4>
    ...
    <form method="POST" action="">
        <table class="table">
            <tr>
                <th>Nom</th>
                <td><input class="form-control border-0" type="text" name="nom" value="<?php echo htmlspecialchars($nom); ?>" readonly></td>
            </tr>
            <tr>
                <th>Prénom</th>
                <td><input class="form-control border-0" type="text" name="pre" value="<?php echo htmlspecialchars($pre); ?>" readonly></td>
            </tr>
            <tr>
                <th>Email</th>
                <td><input class="form-control border-0" type="email" name="email" value="<?php echo htmlspecialchars($email); ?>" readonly></td>
            </tr>
            <tr>
                <th>Téléphone</th>
                <td><input class="form-control border-0" type="tel" name="tel" value="<?php echo htmlspecialchars($tel); ?>" readonly></td>
            </tr>
            <tr>
                <th>Âge</th>
                <td><input class="form-control border-0" type="number" name="age" value="<?php echo htmlspecialchars($age); ?>" readonly></td>
            </tr>
            <tr>
                <th>Genre</th>
                <td>
            ...
            <tr>
                <th>Mot de passe</th>
                <td><input class="form-control border-0" type="password" name="pwd" value="<?php echo htmlspecialchars($pwd); ?>" readonly></td>
            </tr>
        </table>
    </form>
</div>
